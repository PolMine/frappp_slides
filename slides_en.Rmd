...
* **Plenary protocols are a crucial language ressource of parliamentary democracy:**<br/>Governance and hollowing out - parliaments still matter.

* **Datafication(= XMLification) of plenary protocols required:**<br>html/txt/pdf are not enough to leverage the potential of text mining/computer-assisted content analysis.

* **Lowering the marginal costs of preparing corpora of parliamentary protocols:**<br/>Technical requirements / programming skills to prepare high-quality corpora not to be underestimated.<br/>

* **"Framework for Parsing Plenary Protocols" (R package _frapp_) ensures total replicability:**<br/>Replication as a precondition for quality management and improving the data from version to version.
...
* **Use case for 'frappp':** Protocols of the UN General Assembly as the most universal "parliament" we have.


## Basic Techniques of corpus preparation

* OCR (Optical Character Recognition)
* XML (eXtensible Markup Language)
* TEI (Text Encoding Initiative)
* regular expressions (automatisiertes pattern matching)
...
## The TEI standard | GermaParlTEI as an example {.smaller}
 
* consensus on TEI (cp. ParlaCLARIN Workshops, "Reichstagsprotokolle" of BBAW) - but no common in political science

* TEI-XML is useful for standardisation, a data exchange format, to ensure interoperability (cp. FAIR principles) - it is not necessarily useful (= slow) for analysis
...
* OOP (object-oriented programmint) is fundamental for the design of the _frappp_-package. The principle of **inheritance** is a basis for efficiency and flexibility. The design can be transferred to Python, Java etc.
...
* The _frappp_ package opts for the R6 class system:
  ...
  - R6 classes can be extended across packages: The _frappp_ package is intended to serve as a basis for substantice projects of corpus preparation.

* The core class is the  _PlenaryProtocolParser_. It is available after loading the package.
...
* Need to define supplementary functions to augment and standardize matched information.
...
* According to our experience, the quest for the ideal regular expression that never causes a mismatch is futile. So we may define a vector with mismatches.
...
## Configuring the parser II: Agenda items & stage instructions {.smaller}
...
* And we define regular expressions to detect stage instructions.
...
* Extracting the metadata is still necessary (not shown here). We extract it from the original pdf document.
...
## Processing a single document (using method chaining)

* The following standardized chain of processing can now be executed ("method chaining").
...
* The `VerbatimRecordUNGA` class is an "engine" for processing a document.
...
```{r data_repoirt_unga, eval = FALSE, echo = FALSE, message = FALSE}
par(mar = c(0,0,0,8))
...
* Reducing the marginal cost for processing material: *frappp* tries to supprt social science to gain a "data-rich future" [@King2011EnsuringTD]

* 100% replicability of research data:
  - complete transperency of data preparation´.
  - preconditions fulfilled for versioning data and for quality management.
...
* frappp oder frapp (framework for parsing protocols): Applicability on *any* protocol (not just plenary sessions, but also committees)
